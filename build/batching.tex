...
If we implement the downward-percolation phase directly as outlined in the three-element example above, retrieving every output inversion requires $n$ products each composed of $n-1$ multiplications. These $n$ products are each also multiplied by $a^{-1}$. This multiplication by $a^{-1}$ can be added to our $n-1$ inversion count resulting in $n$-many products composed of $n$ multiplications; bringing the complixity of the downward-percolation phase to $\mathcal{O}(n^2)$.
...
We provide an algorithm under the name of \textbf{BatchedInv} implementing this generalization to $n$-many elements (See Algorithm \ref{alg:batchedinv}). In Algorithm \ref{alg:batchedinv}, lines 1--3 implement the upward-percolation phase. Line 4 carries out the second phase: the inversion of $a_{n-1}$. The third and final stage, downward-percolation, occurs from lines 5 to 7.
...
And so, this procedure can be used to reduce $n$-many $\mathbb{F}_{p^{2}}$ inversions to the following operations:
...
\end{itemize}
To measure the complixity in terms of field operations, denoted $C_{\text{attempt}}$, we can surmize the the total operation count as:
$$
C_{\text{attempt}} = (n\textbf{M}) + (2\textbf{s} + \textbf{a} + \textbf{i} + 3\textbf{m}) + (2n\textbf{M})
$$
$$
C_{\text{attempt}} = 3n\textbf{M} + 2\textbf{s} + \textbf{a} + \textbf{i} + 3\textbf{m}
$$
...
$$
C_{\text{attempt}} \approx 3n\textbf{M} + 5\textbf{m} + \textbf{a} + \textbf{i} 
$$
We note here that an $\mathbb{F}_{p^2}$ multiplication (\textbf{M}) is performed simply by means of 4 $\mathbb{F}_{p}$ multiplications (again, recall the multiplcation of complex numbers). So we have $\textbf{M} = 4\textbf{m}$, and can further simplify $C_{\text{attempt}}$:
$$
C_{\text{attempt}} \approx (12n + 5)\textbf{m} + \textbf{a} + \textbf{i}
$$
Finally we've simplified $C$ and $C_{\text{attempt}}$ to forms that are more easily compared. Let $\textbf{P}$ denote the proposition that $C$ runs in fewer operations than $C_{\text{attempt}}$ 
$$
\textbf{P} \equiv C < C_{\text{attempt}}
$$
$$
...
The following are all measure in clock cycles, as the computed average of 1000 distinct executions:
