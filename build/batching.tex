...
\noindent
\code{j\_inv} is a function returning the $j$-invariant of a curve which, as the reader may recall, is used in the computation of the shared secret. If we refer back to our definitions of \code{Sign} and \code{Verify} (Algorithms 9 and 10, respectively) we note that \code{Sign} contains a call to \code{SecretAgreement\_B} in every iteration of its for-loop. Similarly, \code{Verify} contains a call to \code{SercretAgreement\_A} in roughly have of the iterations of its for-loop, and a call to \code{SecretAgreement\_B} in the remaining iterations; totalling still to $248$ secret agreement computations. As expected, this means that \code{isogney\_sign} and \code{isogeny\_verify} contain calls to these secret agreement functions, illustrating the presence of 1 \code{j\_inv} function call (as by extension, 1 extension field inversion,) in every signing and verification thread.\\

\noindent
\code{inv\_4\_way} is a function which takes 4 $\mathbb{F}_{p^2}$ elements and returns each elements inversion by means of calculating only one inversion (via the same method outlined by \textbf{BatchedInversion}). This function is used in the key generation to process to invert the Z-values of the public key values; $\phi(P)$, $\phi(Q)$, and $\phi(P-Q)$), so that they can be converted from projective to affine representation. Because every \code{sign\_thread} execution represents \bob's key exchange with a distinct and random \randall, \code{KeyGeneration\_A} must be called in each thread to generate \randall's public and private keys. This results in another candidate batch of size 248 for batched partial inversion.\\
...
\begin{tikzpicture}
    ...
    \path (-1,-4) node[block1] (G) {\code{SecretAgreement\_B}}
          ...
          (9,-4) node[block1] (I) {\code{SecretAgreement\_A}};
	\draw (-1,-6) node[block2] (C) {\code{j\_inv}};
	\draw (4,-6) node[block2] (C) {\code{inv\_4\_way}};
	\draw (9,-6) node[block2] (C) {\code{j\_inv}};
...
In order to implement \code{pb\_inv} in these functions, we modified every function along the call stack leading up to \code{j\_inv} and \code{inv\_4\_way}. Our modifications allowed these functions to optionally pass a C struct we defined to hold all the information necessary for a successfull execution of \code{pb\_inv}. Included in this struct is a semaphore so that each thread knows to wait until the batch has been filled (248 elements in the signing case, ~128 in the verification cases) A mutex lock can also be found in this struct, allowing \code{j\_inv} and \code{inv\_4\_way} to increment the size of the batch safely across threads. 

We also added the functions \code{j\_inv\_batch} and \code{inv\_4\_way\_batch} into \code{ec\_isogeny}. These functions are simple revisions of \code{j\_inv} and \code{inv\_4\_way} which behave the same 
