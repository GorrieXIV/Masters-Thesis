...
Let $A$ represent the performance of \textbf{PartialInv}, in the format outlined above. We have
$$
A = 2\textbf{s} + \textbf{a} + \textbf{i} + 3\textbf{m}
$$

In some contexts, computing squares can be done more efficiently than the multiplication of two arbitrary elements. A noteworthy example of this is would be the binary field ($\mathbb{F}_2$) where squaring a number is equivalent to simple performing a bit-shift. However, because we are working in the quadratic extension of some prime field $\mathbb{F}_p$ for a large prime $p$, we can assume that computing the square of some arbitrary element $x$ is no more or less efficient than simply computing $x \cdot x$. With this in mind, we can further simplify $A$.
$$
A = 5\textbf{m} + \textbf{a} + \textbf{i}
$$
...
We will refer to this roughly-sketched procedure as $\textbf{BatchedInv}_0$. Let $B_0$ denote the performance of $\textbf{BatchedInv}_0$ in the format outlined above. We have, then, that 
$$
...
In comparing the performances of \textbf{BatchedInv} and $\textbf{BatchedInv}_0$, we see that $B < B_0$ holds when the following holds:
$$
...
\end{algorithm}
\noindent
If we sum of operations in \textbf{PartialBatchedInv}$_0$, we have the following: 
\begin{itemize}
...
$$
We note here that an $\mathbb{F}_{p^2}$ multiplication (\textbf{M}) is performed simply by means of 4 $\mathbb{F}_{p}$ multiplications (again, recall the multiplcation of complex numbers). So we have $\textbf{M} = 4\textbf{m}$, and can further simplify $C_0$:
$$
C_0 \approx (12n + 5)\textbf{m} + \textbf{a} + \textbf{i}
$$
Finally we've simplified $C$ and $C_0$ to forms that are more easily compared. Let $\textbf{P}$ denote the proposition that $C$ runs in fewer operations than $C_0$:
$$
\textbf{P} \equiv C < C_0
$$
$$
...
For the radical skeptics, we can check the validity of this inequality at least contextually. We can verify this by measuring and comparing the execution times of the \sidh addition and multiplcation functions we will be using for our implementation.

When doing so (using the arithmetic test cases included in \code{arith\_tests.c} by Microsoft Research) we arrive at the following measurements:

\begin{center}
\begin{tabular}{@{}lllll@{}}
	\toprule
	Operation & \sidh function & performance in clock cyckes\\
	\midrule
	$\mathbb{F}_p$ addition & \code{fpadd751} & 206\\
	$\mathbb{F}_p$ multiplication & \code{fpmult751\_mont} & 1009\\
	\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}lllll@{}}
	...
	\midrule
	$\mathbb{F}_p$ inversion & \code{fpinv751\_mont} & 826228\\
	$\mathbb{F}_{p^2}$ addition & \code{fp2add751} & 172\\
	$\mathbb{F}_{p^2}$ multiplication & \code{fp2mult751\_mont} & 2793\\
	$\mathbb{F}_{p^2}$ inversion & \code{fp2inv751\_mont} & 829786\\
	\bottomrule
\end{tabular}
\end{center}

All of these results are computed as the average over 100 distinct applications. Furthermore, because they are measured in clock cycles they are independent of any CPU clock rate. Because of this they are indicative of the complexity of each operation, opposed to the performance of these operations on any one particular machine.

We now use these results to compare \textbf{PartialBatchedInv}, \textbf{PartialBatchedInv}$_0$, and unbatched inversion. 

\section{Implementation Details}
\label{sec:pbinvimplementation}

We will now take the work of the previous subsection and explain in detail how it can be applied to the Yoo et al. signature layer of \sidh. We will begin by exploring potential bottlenecks in the protocols performance, specifically in search of execution paths in the codebase where batching inversions could offer a performance increase. We will discuss properties of the signature scheme that can be leveraged to optimize the performance increases offered by \textbf{PartialBatchedInv}.

After this breif mapping of \sidh, we move into the lower-level details of our procedures implementation. We transcribe \textbf{PartialBatchedInversion} to its C variant, \code{pb\_inv}, which is defined almost entirely by means of the \sidh API. We will discuss some design specifics of \code{pb\_inv}, and look breifly at the security of the function with respect to the signature scheme.
