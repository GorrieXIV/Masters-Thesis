...
In the subsections to come, when we are working at the level of field arithmetic we will denote the first and second portions of an arbitrary $x \in \mathbb{F}_{p^{2}}$ as $x_{a}$ and $x_{b}$ respectively. Recall from \ref{subsec:algebra} that both $x_{a}$ and $x_{b}$ are valid $\mathbb{F}_{p}$ elements. 

\subsection{$\mathbb{F}_{p^{2}}$ Inversions done in $\mathbb{F}_{p}$}

There is a simple way in which we can perform one $\mathbb{F}_{p^{2}}$ by means of doing a $\mathbb{F}_{p}$ inversion. Consider the following:

\begin{algorithm}
\caption{-- \textbf{PartialInv($\mathbb{F}_{p^{2}}$ $x$, $\mathbb{F}_{p^{2}}$ $x^{-1}$)}}\label{alg:partialinv}
\begin{algorithmic}[1]
...
Using this technique we can perform 1 $\mathbb{F}_{p^{2}}$ inversion with the following operations: 
\begin{center}
\begin{itemize}
\item 2 $\mathbb{F}_{p}$ squarings
\item 1 $\mathbb{F}_{p}$ additions
\item 1 $\mathbb{F}_{p}$ inversion
\item 3 $\mathbb{F}_{p}$ multiplications
\end{itemize}
\end{center}

Why then, the reader may ask, is this set of field operations more efficient than a single extension field inversion? 
...
This technique, which we will shorthand \textbf{BatchedInv}, was outlined in Shacham and Boneh in \cite{batching}. Shacham and Boneh allude that their work is strongly connected to the earlier efforts of Amos Fiat in batching multiple RSA decryptions.  

\begin{algorithm}
\caption{-- \textbf{BatchedInv($\mathbb{F}_{p^{2}}$ $x$, $\mathbb{F}_{p^{2}}$ $x^{-1}$, $\mathbb{Z} n$)}}\label{alg:batchedinv}
\begin{algorithmic}[1]
\State $den \gets x_{a}^{2} + x_{b}^{2}$
