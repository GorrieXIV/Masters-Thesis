...
$$
One could theorize an attempt to resolve this comparison analytically. This could be done, for example, by reducing $\mathbb{F}_{p^2}$ multiplications to a series of $\mathbb{F}_{p^2}$ additions (not unlike how performing $a \cdot b$ for $a,b \in \mathbb{Z}$ can be equivalently thought of as doing $a$-many additions of $b$). This approach turns out to be inaccurate, however, because while working in finite fields the performance of multiplication depends to be somewhat context sensitive.[foot note or something?]
