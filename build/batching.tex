...
In converting to the base field (beginning at line ) we are peforming line 1 of Algorithm \ref{alg:partialinv} (as outlined in Subsection \ref{subsec:partialinv}) for all elements in the batch. This constructs the ``denominator" for each element $x_i$ as if we were going to compute each inverse individually by means of $x_i^{-1} = \{\frac {(x_i)_{a}}{(x_i)_{a}^2 + (x_i)_{b}^2}, \frac {-(x_i)_{b}}{(x_i)_{a}^2 + (x_i)_{b}^2}\}$. This memory cost of this section is $2n$ \code{felm\_t}'s, in space where $n$ is the number of inputs
\\
...
\begin{tikzpicture}
	...
	\draw (12,3.5) node[batchblock] (BATCH1) {batch 1};
	\draw (12,1.5) node[batchblock] (BATCH2) {batch 2};
