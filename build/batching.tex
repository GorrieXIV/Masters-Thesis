...
As mentioned above, \code{pb\_inv} is constructed by combining two distinct techniques. Both of these techniques improve the efficiency of computing field element inversions: the first is specific to extension fields (in our case, $\mathbb{F}_{p^{2}}$ elements,) but the second is a technique applicable to general field element inversions.

We will begin with a dissection of these two techniques, starting first with the ``partial" inversion technique and then looking at batched inversions. The definitions we will give for these techniques below are given at the level of field arithmetic. When we proceed, however, to sketch \code{pb\_inv} we will do so using the \sidh API.

\subsection{$\mathbb{F}_{p^{2}}$ Inversions done in $\mathbb{F}_{p}$}

\begin{algorithm}
\caption{-- \textbf{Partial\_Inversion($\mathbb{F}_{p^{2}}$ \code{a}, $\mathbb{F}_{p^{2}}$ \code{a}$^{-1}$)}}\label{alg:partialinv}
\begin{algorithmic}[1]
\State $\mathbb{F}_{p} \code{den} \gets \code{a[0]}^{2} + \code{a[1]}^{2}$
\State $den_{inv} \gets \texttt{inv(a[n-1])}$
...
\begin{algorithm}
\caption{\code{pb\_inv}}\label{alg:partialbatchedinversion}
\begin{algorithmic}[1]
