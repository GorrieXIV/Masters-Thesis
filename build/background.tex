...
Another important implementation detail is the seperation of the signing procedure into two functions: \code{isogeny\_sign} and \code{sign\_thread}. Because all of the $2\lambda$ iterations are entirely independent of the others, the signing and verification proceedures present themselves as embarrassingly parallel. \code{isogey\_sign} acts as the entry point for \code{Sign} and spawns a POSIX thread for every instance of the for-loop; each one calling \code{sign\_thread} which runs performs \bob's interaction with \randall. 
