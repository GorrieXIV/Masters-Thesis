...
The public parameters used in $\Sigma'$ are the same as outlined above for all of the protocols found in \cite{djp}. Namely, we have $p = \laea\lbeb \cdot f \pm 1$ where $\laea = 2$, $\lbeb = 3$, and $f$ is a cofacter such that $p$ is prime. We also set as parameter the curve $E_0$ such that $\#(E_0(F_{p^2})) = (\laea\lbeb)^2$. And again, we include the sets of points $(\genpa, \genqa)$ and $(\genpb, \genqb)$ generating $E_0[\laea]$ and $E_0[\lbeb]$ respectively.


\subsection{Algorithmic Definitions}

It will be useful for us to outline in more detail the procedures of $\Sigma'$, at the very least to ease the transition into our discussion of the C implementation. 
...
\end{algorithmic}
\end{algorithm}
\label{fig:keygencode1}
\end{figure}

stuff or something haha idk

\begin{figure}
\begin{algorithm}
\caption{-- \textbf{Sign}(sk, $m$)}\label{euclid}
\begin{algorithmic}[1]
