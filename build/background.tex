...
\For{\texttt{i = 0..2$\lambda$}}
	...
	\If{$ch_{i,J_{i}} = 0$}
		...
		\State \textbf{check} that $\Pi_{\text{SIDH}}$:\textbf{SecAgr($\cyr$, $\pb$)} successfully terminates
	\Else
		\State Parse $\pr(\rb) \gets resp_{i,J_{i}}$
		\State \textbf{check} that $\Pi_{\text{SIDH}}$:\textbf{SecAgr($\pr(\rb)$, $\pr$)} successfully terminates
	\EndIf
\EndFor
