...
From the perspective of Alice executing $\Pi_{kex}$ with Bob, the procedure looks like the following:\\

\noindent
$\textbf{KeyGen}(1^\lambda)$: A probabilistic algorithm with input $1^\lambda$ and output $(sk_{Alice},pk_{Alice})$. Typically $pk_{Alice}$ is the image of $g(sk_{Alice})$, where $h$ is some \emph{one-way} function.\\ 

\noindent
In between execution of $\textbf{KeyGen}$ and $\textbf{SecAgr}$, Alice will send Bob her computed value $pk_{Alice}$ and Bob will send $pk_{Bob}$ over an insecure channel.\\

\noindent
$\textbf{SecAgr}(sk_{Alice}, pk_{Bob})$: A deterministic algorithm with input $sk_{Alice}$ and $pk_{Bob}$ and output $k_{Alice} \in \{0,1\}^\lambda$.\\
...
Assume Alice wishes to confirm the identity of Bob. The idea behind interactive identification protocols is to offer Bob some way of proving to Alice (or anyone) that he has knowledge of some secret which \textbf{only} Bob could possess. The goal, of course, being to accomplish this without openly revealing this secret, so that it can continue to be used as an identifier for Bob. 

An identification scheme (or otherwise ``proof of knowledge" or ``proof of identity") $\Pi_{id}$ is composed of by the tuple of polynomial-time algorithms (\textbf{KeyGen}, \textbf{Prove}, \textbf{Verify}). $\Pi_{id}$ is an interactive protocol, wherein the \emph{prover} (Bob, for example) executes \textbf{Prove} and the \emph{verifier} (Alice) executes \textbf{Verify}.

\noindent
$\textbf{KeyGen}(1^\lambda)$: A probabilistic algorithm with input $1^\lambda$ and output $(sk,pk)$.\\ 

\noindent
At this point, Bob will send the Alice his public key $pk$ and Alice will respond with a \emph{challenge} value $ch$.\\

\noindent
$\textbf{Prove}(sk, ch)$: A deterministic algorithm run by Bob with input $sk$ (Bob's secret key) and $ch$ (challenge) and output $resp$.\\

\noindent
$\textbf{Verify}(pk, ch, resp)$: A deterministic algorithm run by Alice with input $pk$ (Bob's public key), $ch$, and $resp$ and output $b \in {0,1}$. Bob has successfully proven his identity to Alice if $b = 1$.\\

\subsection{Signature Schemes}

There are two main differences between signature schemes and identification schemes. First, because we wish to authenticate Bob on every message sent to Alice we need to include Bob's message $m$ in
