...
$$
This representation encapsulates elliptic curves defined over any field. If, however, we are in the dicussion of curves defined over some field $K$ such that $\text{char}(K) > 3$[ref], then the more compact form $y^2 = x^3 + ax + b$ can be applied. In this dissertation we will default to this second representation, as the schemes with which we are concerned will always be defined over $\mathbb{F}_p$ for some large prime $p$.
...
\begin{tikzpicture}[scale=.75]
  ...
  \end{scope}
\end{tikzpicture}
\caption{$+$ acting over points $P$ and $Q$ of some curve $E$.}
\label{fig:groupop}
\end{figure}
