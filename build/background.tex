...
\noindent
\emph{Montgomery Arithmetic}\label{snip:montgomery}.
...
\noindent
\emph{Projective Coordinates}\label{snip:projspace}.
...
The key exchange procedures found in \code{kex.c} are composed enrirely of calls to \code{fpx.c} and \code{ec\_isogeny.c} functions, modulo some basic branching logic.
...
\begin{tabular}{|c|c|c|c|}
	...
	\code{from\_fp2mont(...)} & & &\\
	\hline
	\code{fp2inv751\_mont\_bingcd(...)} & yuh & yuh & yuh \\
	\hline
	\code{fp2inv751\_mont(...)} &  & hh &\\
	\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{|c|c|c|c|}
	...
	\code{inv\_4\_way} & & &\\
	\hline
	\code{generate\_2\_torsion\_basis} & & &\\
	\hline
	\code{generate\_3\_torsion\_basis} & & &\\
	\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{|c|c|c|c|}
	...
	Function & Description & Input & Output\\
	\hline
	\code{KeyGeneration\_A} & yuh & yuh & yuh \\
	\hline
	\code{KeyGeneration\_B} &  & hh &\\
	\hline
	\code{SecretAgreement\_A} & & &\\
	\hline
	\code{SecretAgreement\_B} & & &\\
	\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{|c|c|c|c|}
	...
	Function & Description & Input & Output\\
	\midrule
	\code{isogeny\_keygen} & yuh & yuh & yuh \\
	\hline
	\code{isogeny\_sign} &  & hh &\\
	&  & &\\
	\hline
	\code{sign\_thread} & & &\\
	\hline
	\code{isogeny\_verify} & & &\\
	\hline
	\code{verify\_thread} & & &\\
	\bottomrule
\end{tabular}
...
If we transcribe the procedures $\Sigma'$:\textbf{Sign} and $\Sigma'$:\textbf{Verify} as described above to the language of the \sidh API, we have in essense the following:\\

\begin{algorithm}
\caption{-- \code{Sign($sk_{\rb}$, $m$)}}\label{euclid}
\begin{algorithmic}[1]
\For{\texttt{i = 1..2$\lambda$}}
	\State $(sk_{\cyr} = \cyr, pk_{\cyr}) \gets \code{KeyGeneration\_A()}$
...
\begin{algorithm}[H]
\caption{-- \code{Verify($pk = \pb$, $m$, $\sigma$)}}\label{euclid}
\begin{algorithmic}[1]
\State $J_{1} \parallel ... \parallel J_{2\lambda} \gets$ \code{keccak(com, $m$, $h$)}
\For{\texttt{i = 0..2$\lambda$}}
	\State \textbf{check} $h_{i,J_{i}} = G(resp_{i,J_{i}})$
	\If{$ch_{i,J_{i}} = 0$}
		\State Parse $(\cyr,\pb(\cyr)) \gets resp_{i,J_{i}}$
		\State \textbf{check} $(\cyr, \pr)$ is a valid output of \code{KeyGeneration\_A($\lambda$, $\alice$)}
		\State \textbf{check} that \code{SecretAgreement\_A($\cyr$, $\pb$)} successfully outputs an isogeny with co-domain $E_{2}$
	\Else
		\State Parse $\pr(\rb) \gets resp_{i,J_{i}}$
		\State \textbf{check} that \code{SecretAgreement\_B($\pr(\rb)$, $\pr$, $\pr(\rb)$)} successfully outputs an isogeny with co-domain $E_{2}$
	\EndIf
\EndFor
...
Outside of simply replacing $\Pi_{\text{SIDH}}'$ procedure calls with \sidh functions, the reader may notice additional differences between \code{Sign} and \code{Verify} and their $\Sigma'$ counterparts. These differences are surmized below:

Namely, Yoo et al. have chosen to exclude the challenge bit $ch$ in the \sidh implementations of these functions, this is justified because
