...
\noindent
The mapping between \code{Signature} and $\sigma$ is straight forward.
...
We use the compression technique covered in the previous section to compress every element of \code{psiS} individually.

Recall Figure \ref{fig:parallelcall} which outlines at a high level the execution flow of the \sidh signing and verification procedures. First, to sign we have have $2\lambda$ threads running \code{sign\_thread} in parallel.  
