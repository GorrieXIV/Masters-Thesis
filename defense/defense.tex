...
\keygen: \alice chooses a random number $\ma \in \mathbb{Z}/\laea\mathbb{Z}$ and computes the isogeny $\pa: E \rightarrow E_{\ba}$ where $E_{\ba} = E/\langle \genpa, [\ma]\genqa \rangle$. \bob does the same for his parameters. The resulting private and secret keys are:
\begin{center}
$\ska = (\ma,\na)$\\
...
\vspace{10px}
\secagr: \alice computes the isogeny $\pap: E_{\rb} \rightarrow E_{\ba\rb}$ and \bob computes $\pbp$ analogously. The resulting shared secret value is some representation of
$$
E_{\ba\rb} = E/ \langle \genpa + [\ma]\genqa, \genpb + [\mb]\genqb \rangle
$$
\end{frame}
...
Also, $n$ many $\mathbb{F}_{p^2}$ inversions can be reduced to one $\mathbb{F}_{p^2}$ inversion and $3(n-1)$ $\mathbb{F}_{p^2}$ multiplications.\\
