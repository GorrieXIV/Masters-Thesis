...
\titleline
\tikz[remember picture,overlay] {
  \draw[black] ([yshift=-3.1cm,xshift=1.5cm]current page.north west)
            -- ([yshift=-3.1cm,xshift=\paperwidth/2+1.4cm]current page.north west);
}
\tableofcontents % Throughout your presentation, if you choose to use \section{} and \subsection{} commands, these will automatically be printed on this slide as an overview of your presentation
\end{frame}
...
\tikz[remember picture,overlay] {
  \draw[->,black,thick] ([yshift=3.05cm,xshift=\paperwidth/2-1cm]current page.south west)
            -- ([yshift=3.05cm,xshift=\paperwidth/2+0.5cm]current page.south west);
  \draw[] ([yshift=3.4cm,xshift=\paperwidth/2-0.5cm]current page.south west) node[right] {$\phi$};
}
...
\begin{columns}[c]
  ...
  \begin{tikzpicture}[scale=0.5]
    \plotcurve{-2}{2}
    \draw[fill] (-1.73,0.531) circle (0.1) node[right] {$P$};
    \draw[] (-4,3) node[right] {$E_1$:};
  \end{tikzpicture}

  \column{.45\textwidth}
  \begin{tikzpicture}[scale=0.5]
    \plotcurve{1}{1}
    \draw[fill] (0.73,1.492) circle (0.1) node[right] {$\phi(P)$};
    \draw[] (-4,3) node[right] {$E_2$:};
  \end{tikzpicture}
\end{columns}
\end{frame}
...
\topline
\titleline
Isogenies have the following two properties
\begin{itemize}
...
\vspace{20px}
They can also be uniquely identified by finite subgroups of $E$.
\begin{tcolorbox}
  \begin{lemma}[Uniquely identifying isogenies]
  \label{lem:isogkern}
    Let $E$ be an elliptic curve and let $S$ be a finite subgroup of $E$. There is a unique elliptic curve $E'$ and a seperable isogeny $\phi: E \rightarrow E'$ satisfying $\text{ker}(\phi) = S$.
  \end{lemma}
\end{tcolorbox}
...
\vspace{15px}
These protocols can be defined by a tuple of algorithms $(\keygen, \secagr)$.\\

\vspace{15px}
Generally, \alice and \bob will both run \keygen to generate their keypairs $(\pka, \ska)$ and $(\pkb, \skb)$, respectively.\\

\vspace{15px}
Then each will run \secagr with their own private key and the others public key to generate an equivalent, shared secret key.\\
...
\topline
\titleline
SIDH is a \textit{role-based} key-exchange protocol where \alice and \bob use elliptic curves as their public keys and isogenies as their private keys.\\
...
\keygen: \alice chooses a random number $\ma \in \mathbb{Z}/\laea\mathbb{Z}$ and computes the isogeny $\pa: E \rightarrow E_{\ba}$ where $\text{ker}(\pa) = \langle \genpa + [\ma]\genqa \rangle$. \bob does the same for his parameters. The resulting private and secret keys are:
\begin{center}
$\ska = (\ma,\na)$\\
...
\vspace{10px}
\secagr: \alice computes the isogeny $\pap: E_{\rb} \rightarrow E_{\ba\rb}$ and \bob computes $\pbp$ analogously. The resulting shared secret value is some representation of $E_{\ba\rb}$ \cite{sidh}.
\end{frame}
...
\begin{enumerate}
\item \bob runs \keygen($1^\lambda)$ to generate his keypair $(\pkb,\skb)$.
...
\begin{frame}
\frametitle{Isogeny-based Interactive Proof of Identity}
\topline
\titleline
The isogeny-based proof of identity protocol outlined by De Feo et al. also consists of algorithms \keygen, \commit, \prove, and \verify.\\

\vspace{25px}
In the scheme, \bob can prove to \alice (or vice-versa) that he is the owner of the keypair $(\pkb, \skb)$ by forming many SIDH secret agreements with an arbitrary, random entity \randall, and showing that he can always come to a valid shared secret with \randall.
\end{frame}
...
\begin{enumerate}
  \item \bob runs \keygen to produce his keypair $(\pkb, \skb)$.
  \item \bob runs \sign($m$, $\skb$) to produce a signature $\sigma$, and sends ($m$, $\sigma$) to \alice.
  \item \alice runs \verify($m$, $\sigma$, $\pkb$) returning 1 if \bob's message was successfully verified and 0 otherwise.
\end{enumerate}
...
\topline
\titleline
The work of Yoo et al. applies the Fiat-Shamir transform to the isogeny-based identification scheme turning it into a general purpose isogeny-based signature scheme.\\

\vspace{20px}
To generate a signature $\sigma$ with $\lambda$-bit security requires running $2\lambda$ SIDH key-exchanges.
\end{frame}
...
\topline
\titleline
A Yoo signature $\sigma$ has the following components:
\begin{itemize}
  \item \textit{com} -- A list of $2\lambda$ pairs of curves,
  \item \textit{ch} -- $2\lambda$ random bits derived from $H$, and
  \item \textit{resp} -- A list of size $2\lambda$ where the entry is either $\pr(\rb)$ or $(\cyr, \pb(\cyr))$.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Isogenies are maps between elliptic curves, and can be uniquely identified by a subgroup of the domain curve.
  \item A key-exchange protocol can be derived with isogenies as private keys and the corresponding codomain curves as public keys.
  \item An isogeny-based identity proving scheme with $n$-bit security can be constructed by performing $2n$ SIDH key-exchanges with a random entity.
  \item Yoo signatures are derived by applying the fiat-shamir transform to this proof of identity protocol.
\end{itemize}
\end{frame}
...
\topline
\titleline
A $\mathbb{F}_{p^2}$ element $q = (q_a, q_b)$ can be inverted using only $\mathbb{F}_{p}$ operations like so:

$$
q^{-1} = (\frac {q_{a}}{q_{a}^2 + q_{b}^2}, \frac {-q_{b}}{q_{a}^2 + q_{b}^2})
$$
\end{frame}
...
Also, $n$ many $\mathbb{F}_{p^2}$ inversions can be reduced to one $\mathbb{F}_{p^2}$ inversion and $3(n-1)$ $\mathbb{F}_{p^2}$ multiplications \cite{batchingssl}.\\
...
\tikz[remember picture,overlay] {
  \draw [decorate,decoration={brace,amplitude=6pt},yshift=0pt] (5.3,4.7) -- (5.3,3.5) node [black,midway,right,xshift=0.5cm] {upward-percolation};

  \draw[->,black,thick] (4.6,3.1) -- (5.7,3.1);
  \draw[] (5.8,3.1) node[right] {inversion phase};

  \draw [decorate,decoration={brace,amplitude=6pt},yshift=0pt] (5.3,2.7) -- (5.3,1.5) node [black,midway,right,xshift=0.5cm] {downward-percolation};
}
\end{frame}
...
\titleline
\smallfont
These two techniques can be combined in the following way, yielding the fastest method of inverting $n$-many arbitrary $\mathbb{F}_{p^2}$ elements.
...
\tikz[remember picture,overlay] {
  \draw [decorate,decoration={brace,amplitude=6pt},yshift=0pt] (5.6,4.7) -- (5.6,3.95) node [black,midway,right,xshift=0.5cm] {upward-percolation};

  \draw[->,black,thick] (4.9,3.8) -- (6,3.8);
  \draw[] (6.1,3.8) node[right] {inversion phase};

  \draw [decorate,decoration={brace,amplitude=6pt},yshift=0pt] (5.6,3.6) -- (5.6,2.7) node [black,midway,right,xshift=0.5cm] {downward-percolation};
}
\end{frame}
...
$$
\pa(\genpb) = \alpha_P R_1 + \beta_P R_2
$$
$$
\pa(\genqb) = \alpha_Q R_1 + \beta_Q R_2
$$
...
\topline
\titleline
Costello et al. note that in public key compression, because one of $\alpha_P$ and $\beta_P$ will be non-zero, one will be able to be normalized out.\\

\vspace{25px}
This notion of normalizing $P = \alpha R_1 + \beta R_2$ to $P = R_1 + \alpha^{-1}\beta R_2$ is also utilized in our implementation.
...
\topline
\titleline
We use these SIDH public key compression techniques to compress every EC point component of a Yoo signature which is the result of applying \randall's isogeny to the signers private key.\\
...
\begin{itemize}
  \item \textit{com} -- A list of $2\lambda$ pairs of curves,
  \item \textit{ch} -- $2\lambda$ random bits derived from $H$, and
  \item \textit{resp} -- A list of size $2\lambda$ where the entry is either \hl{$\psi_{R}(B)$} or $(\cyr, \pb(\cyr))$.
\end{itemize}
\end{frame}
...
\topline
\titleline
This allows us to transmit every $\psi(S)$ as two elements of $\mathbb{Z}/n\mathbb{Z}$ (with $n = \lbeb$ when \bob is signing and $\laea$ when \alice is signing): $\psi(S) = \alpha R_1 + \beta R_2$.\\

\vspace{20px}
We then normalize to $\psi(S) = R_1 + \alpha^{-1}\beta R_2$ and are able to send $\alpha^{-1}\beta$ in the place of $\psi(S)$. For each of these we also send a bit $b$ denoting whether $\alpha$ or $\beta$ was inverted.
\end{frame}
...
\begin{itemize}
  \item Continue improving code performance: 1 inversion call in signing and verifying paths that has yet to be processed for batched inversions.
  \item Continue efforts to test modifications for correctness and application security.
  \item Work to introduce Yoo signatures and our improved implementations into the world of open source isogeny-based cryptosystems.
\end{itemize}
\end{frame}
...
\begin{thebibliography}{99} % Beamer does not support BibTeX so references must be inserted manually as below
  ...
  \bibitem[OQS, 2018]{oqs} Douglas Stebila and Michele Mosca (2018)
  \newblock Open Quantum Safe
  \newblock available at \url{https://openquantumsafe.org}
  \bibitem[PQC, 2018]{pqc} Daniel J. Bernstein (2018)
  \newblock libPQCrypto
  \newblock available at \url{libpqcrypto.org}
\end{thebibliography}
}
\end{frame}
...
\footnotesize{
\begin{thebibliography}{99} % Beamer does not support BibTeX so references must be inserted manually as below
  \bibitem[SB, 2001]{batchingssl} Hovav Shacham and Dan Boneh (2001)
  \newblock Improving SSL Handshake Performance via Batching
  %\newblock \emph{Journal Name} 12(3), 45 -- 678$
  \bibitem[FJP, 2012]{sidh} Luca De Feo, David Jao, Jerome Plut (2012)
